...
const SPEECH_PROVIDER_NAME = "microsoft"; // Providers are "microsoft" or "google"
...
const createCall = async (callSid) => {
  if (syncClient.connectionState != "connected") {
    console.warn(`Sync client not connected, unable to create call ${callSid}`);
    return null;
  }
  ...
  return syncClient
    .stream({ id: "FLEX_ASSIST_" + callSid, mode: "open_or_create", ttl: 3600 })
    .then((stream) => {
      console.log(
